...
\begin{itemize}
	...
	\item \textbf{Burst Mode} In diesem Modus wird, im Gegensatz zum FCM, bei kleinen Lasten die MOSFETs nur in \textit{Bursts}, also kleinen \textquotedbl{}Paketen\textquotedbl{} aus sehr kurzen Sequenzen geschaltet, welche danach von Ruhezeiten gefolgt werden. Abbildung \ref{pic:burstmode_vs_forcedcontinuous} zeigt den Signalverlauf von FCM und Burst Mode. Dieser Modus hat den Vorteil, dass die Spule nur genau dann von Strom durchflossen wird, wenn dieser auch wirklich zur Spannungsstabilisierung beiträgt. Somit muss bei geringen Lastströmen auch kein Strom unnötig durch den unteren MOSFET fliessen.
...
Mittels einem \cite{AnalogDiscovery3} konnte nun das Aufstartverhalten ausgemessen werden. Dieses ist ein USB-Oszilloskop mit analogen und digitalen Eingängen. Da die PG\_1 und PG\_2-Signale als Testpins vorhanden waren, konnten diese direkt mit den digitalen Eingängen DIO0 und DIO1 verbunden und darauf getriggert werden. Danach wurde das in \ref{ext_supply} bestimmte Netzteil angeschlossen\footnote{Zuvor wurde beobachtet, dass ein Labornetzteil die +24V-Speisespannung über einige ms Hochfährt und dadurch die Stabilität der Speisungen beeinflusst.}, der Hauptschalter SW1 umgelegt und die Speisungen gemessen\footnote{Gemessen mit dem BNC-Adapter. Auflösung: 3.36mV, Genauigkeit: $\pm$ 100mV, CMMR: $\pm$ 0.5\%, DC-Offset Genauigkeit: $\pm$ 100mV $\pm$ 0.5\%. Quelle: \href{https://digilent.com/reference/test-and-measurement/analog-discovery-3/specifications}{Analog Discovery 3 Specifications}}. Abbildung \ref{pic:measurement_setup_supplies} zeigt den Messaufbau und Abbildung \ref{pics:supplies_startup} zeigt das so gemessene Aufstartverhalten aller Speisungen. Es wurde einzig auf der +24V-Speisung einige kleinere Einbrüche beobachtet, welche jedoch keinerlei Überschwinger, Spannungseinbrüche oder andere Störungen auf den restlichen Speisungen verursachten. Dies allerdings nach wie vor ohne Last und MILAN-Modul. Das Resultat wurde daher soweit als zufriedenstellend befunden.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{pictures/Messaufbau_Speisungen.pdf}
	\caption{Der Messaufbau der Speisungsmessungen}
	\label{pic:measurement_setup_supplies}
	\vspace{-4mm}
\end{figure}