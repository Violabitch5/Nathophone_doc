...
Die Footprints aller Bauteile mussten korrekt importiert werden, und die Pin-Nummerierung und Belegung kontrolliert werden. In den Datenblättern des Schaltreglers und der Endstufe waren zudem Empfehlungen zur Platzierung der einzelnen Bauteile enthalten. Dies wurde so weit möglich beachtet.\\Bei der Anordnung der Endstufen ergab es sich, sie am einfachsten Ringförmig angeordnet sind. Es wurde auch beachtet, dass die Speisungspins gegen innen und die Ausgänge alle gegen aussen gerichtet sind. Abbildung \ref{pic:Layout_placement} zeigt die gesamte Platzierung der Bauteile und den Siebdruck.
\begin{figure}[H]
	\centering
...
Aus den beschriebenen Regeln ergaben sich die weiteren Layers. Abbildung \ref{pic:pcb_layers} zeigt die restlichen Layers.
\begin{figure}[H]
	\centering
...
Aus der Platzierung ergab sich, dass die TDM-Signale in S-Form über das ganze PCB geführt werden musste. Hizu kam, dass die Chips jeweils 180° gedreht waren, was dazu führte, dass die Signale sich jeweils überkreuzten. Dazu mussten die Signale zwischen Layer 2 und 3 hin- und hergewechselt werden. Insbesondere das BCLK-Signal ist mit bis zu 25MHz getaktet. Bei diesen Frequenzen müssen Reflektionen beachtet werden. 
...
\begin{itemize}
	...
	\item etc.
\end{itemize}
Dieser Aufbau ist physikalisch gesehen recht komplex. Es standen auch weder die genauen Leitungsparameter (z.B. vom Flachbandkabel) noch Simulationsprogramme zur Verfügung.\\Um jedoch trotzdem eine Aussage über das Reflektionsverhalten machen zu können, wurde in LTSpice der Aufbau mit Lossless Transmission Line-Elementen\footnote{Es standen auch Lossy Transmission Line Elemente zur Verfügung, deren Simulation bzw. Berechnung war allerdings wesentlich komplexer.} nachgebildet.\\Zur Berechnung der Impedanz- und Zeitwerte für Traces und Vias wurden zwei Tool verwendet und die entsprechenden Werte aus Abbildung \ref{pic:pcb_stackup} verwendet:
\begin{itemize}
	\item \textbf{Via Calculator}: \cite{via_impedance_calc}
	\item \textbf{Traces}: Transmission Line Calculator von KiCAD \cite{kicad_transline_calc}. 
\end{itemize}
...
\begin{center}
	\begin{minipage}{\textwidth*7/8}
		\centering
		\vspace{6mm}
		{\large Daher wurden für die TDM-Signale jeweils zwei Serieterminierungen\footnote{Widerstände R19-22 und R39-42} von 40.2Ohm eingefügt.}
		\vspace{6mm}
	\end{minipage}
\end{center}
\begin{figure}[H]
...
\end{figure}
Des weiteren wurde die Länge der TDM-Signaltraces aufeinander abgestimmt. Dies verhindert Laufzeitunterschiede, welches zu falschen Samplewerten führen könnten.
\newpage